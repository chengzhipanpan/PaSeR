...
import pickle
def get_data_from_file(quora_file, useful_id_path):
    with open(quora_file, "r") as f:
        items = f.read().splitlines()
        items = items[1:]
    with open(useful_id_path, "rb") as f:
        useful_id = pickle.load(f)
    # print(useful_id[0])
    # print(type(useful_id[0]))
    all_ques_1 = []
    all_ques_2 = []
    gold_trues = {}
    for item in items:
        _, qid1, qid2, question1, question2, is_dup = item.strip().split("\t")
        all_ques_2.append(question2)
        if int(qid1) in useful_id:
            all_ques_1.append(question1)
        if int(is_dup) == 1:
            if qid1 in gold_trues.keys():
                gold_trues[qid1].append(qid2)
            else:
                gold_trues[qid1] = [qid2]
        else:
            if qid1 not in gold_trues.keys():
                gold_trues[qid1] = []
    return all_ques_1, all_ques_2, gold_trues

if __name__=="__main__":
    example_queries, example_sentences, gold_trues = get_data_from_file("Quora_data/quora_duplicate_questions.tsv", "Quora_data/useful_ids.pkl")
    # example_sentences = [
    ...
    print("Number of queries", len(example_queries))
    print("Number of sentences", len(example_sentences))
    example_queries = example_queries[:50]

    # 我们这里可以直接load QQP的sentences和那个啥。。然后找些top的retrieval.
    # 不过希望把正负对的东西搞出来。

    model_name = "princeton-nlp/unsup-simcse-bert-base-uncased"
    # model_name = "result/unsup-bert-base-uncased_sr_5e-1_only"
